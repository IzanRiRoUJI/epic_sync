...
class CardProvider with ChangeNotifier {
  ...
  CardProvider() {
    _cards = [
      epic_sync.Card(
        ...
        type: epic_sync.CardType.Tarea,
        priority: epic_sync.CardPriority.Media,
        state: epic_sync.CardState.Pendiente,
        backlog: epic_sync.CardBacklog.True,
      ...
      epic_sync.Card(
        ...
        type: epic_sync.CardType.Tarea,
        priority: epic_sync.CardPriority.Alta,
        state: epic_sync.CardState.Proceso,
      ...
      epic_sync.Card(
        ...
        type: epic_sync.CardType.Error,
        priority: epic_sync.CardPriority.Alta,
        state: epic_sync.CardState.Revisar,
        backlog: epic_sync.CardBacklog.False,
      ...
      epic_sync.Card(
        ...
        type: epic_sync.CardType.Tarea,
        priority: epic_sync.CardPriority.Baja,
        state: epic_sync.CardState.Listo,
        backlog: epic_sync.CardBacklog.True,
  ...
  List<epic_sync.Card>? get cards => _cards;

  List<epic_sync.Card> getCards() {
    return _cards!;
  }

  List<epic_sync.Card> getCardsByCategory(epic_sync.CardCategory category) {
  ...
  Map<Int64, int> getCardsByUserWithCategory(epic_sync.CardCategory category) {
    Map<Int64, int> cardsByUser = {};
    for (var card in _cards!) {
  ...
  int getCardStateCount(epic_sync.CardState state) {
    return _cards!.where((element) => element.state == state).length;
  }

  int getCardCategoryCount(epic_sync.CardCategory category) {
    return _cards!.where((element) => element.category == category).length;
  }

  int getCardStateCountByCategory(
      epic_sync.CardCategory category, epic_sync.CardState state) {
    return _cards!
        .where(
  ...
  List<epic_sync.Card> getCardsByCategoryAndState(
      epic_sync.CardCategory category, epic_sync.CardState state) {
    return _cards!
        .where(
            (element) => element.category == category && element.state == state)
  ...
  List<epic_sync.Card> getCardsWithBacklogFalse() {
    return cards!
        .where((card) => card.backlog == epic_sync.CardBacklog.False)
        .toList();
  }
  ...
  int getCardStateCountWithBacklogFalse(epic_sync.CardState state) {
    return cards!
        .where((card) =>
            card.state == state && card.backlog == epic_sync.CardBacklog.False)
        .length;
  }

  int getCardCategoryCountWithBacklogFalse(epic_sync.CardCategory category) {
    return cards!
        .where((card) =>
            card.category == category &&
            card.backlog == epic_sync.CardBacklog.False)
        .length;
  }

  int getCardStateCountByCategoryWithBacklogFalse(
      epic_sync.CardCategory category, epic_sync.CardState estado) {
    int p = _cards!
        .where((element) =>
            element.category == category &&
            element.state == estado &&
            element.backlog == epic_sync.CardBacklog.False)
        .length;
    print(p);
    return p;
  }
  ...
  void toggleCardBacklog(Int64 id, bool backlog) {
    _cards!.firstWhere((element) => element.id == id).backlog =
        backlog ? epic_sync.CardBacklog.True : epic_sync.CardBacklog.False;
    notifyListeners();
  }

  void addCard(epic_sync.Card card) {
    _cards!.add(card);
    notifyListeners();
  }

  void removeCard(epic_sync.Card card) {
    _cards!.remove(card);
    notifyListeners();
  }
  ...
  void updateCard(epic_sync.Card card) {
    _cards!.removeWhere((element) => element.id == card.id);
    _cards!.add(card);
    notifyListeners();
  }

  void moveCardtoState(epic_sync.Card card, epic_sync.CardState newState) {
