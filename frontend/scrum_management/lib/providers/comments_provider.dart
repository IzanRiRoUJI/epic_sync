...
class CommentProvider with ChangeNotifier {
  List<scrum_management.Comment>? _comments;

  CommentProvider() {
    _comments = [
      scrum_management.Comment(
          id: Int64(1),
          idCard: Int64(1),
          idUser: Int64(1),
  ...
  List<scrum_management.Comment>? get comments => _comments;

  void addComment(scrum_management.Comment comment) {
    _comments!.add(comment);
    notifyListeners();
  }

  void removeComment(scrum_management.Comment comment) {
    _comments!.remove(comment);
    notifyListeners();
  }

  void updateComment(scrum_management.Comment comment) {
    final index = _comments!.indexWhere((element) => element.id == comment.id);
    _comments![index] = comment;
    notifyListeners();
  }

  void replyComment(scrum_management.Comment comment) {
    _comments!.add(comment);
    notifyListeners();
  }

  void deleteComment(scrum_management.Comment comment) {
    _comments!.remove(comment);
    notifyListeners();
  }

  List<scrum_management.Comment> getCommentsByCardId(Int64 idCard) {
    return _comments!.where((element) => element.idCard == idCard).toList();
  }

  List<scrum_management.Comment> getParentCommentsByCardId(Int64 idCard) {
    return _comments!
        .where(
  ...
  List<scrum_management.Comment> getCommentsByParentId(Int64 idParent) {
